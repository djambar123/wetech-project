...
def test_html():
    driver = init()
    sleep(3)
    driver.find_element(By.XPATH,"//body/div[@id='root']/div[1]/div[2]/div[2]/div[3]/div[1]/ul[1]/ul[1]/li[1]").click()

    # html title
    title = driver.find_element(By.XPATH, "//h1[contains(text(),'CSS')]").get_attribute("innerText")

    # html link
    html_link = driver.find_element(By.XPATH,"//body/div[@id='root']/div[1]/div[2]/div[1]/section[1]").get_attribute("innerText")

    assert title == "" and html_link == "HTML (HyperText Markup Language) is the most basic building block of the Web. It defines the meaning and " \
...
def test_css():
    driver = init()
    sleep(3)
    driver.find_element(By.XPATH, "//body/div[@id='root']/div[1]/div[2]/div[2]/div[3]/div[1]/ul[1]/ul[1]/li[2]").click()

    # css title
    title = driver.find_element(By.XPATH, "//h1[contains(text(),'CSS')]").get_attribute("innerText")

    # css link
    css_link = driver.find_element(By.XPATH, "//body/div[@id='root']/div[1]/div[2]/div[1]/section[1]").get_attribute(
        "innerText")
...
def test_java_script():
    ...
    # java script title
    title = driver.find_element(By.XPATH, "//h1[contains(text(),'JavaScript')]").get_attribute("innerText")

    # java script link
    java_script_link = driver.find_element(By.XPATH, "//body/div[@id='root']/div[1]/div[2]/div[1]/section[1]").get_attribute(
        "innerText")

    assert title == "JavaScript" and java_script_link == "avaScript (JS) is a lightweight, interpreted, or just-in-time compiled " \
                                                         ...
                                                         "the ECMAScript Language Specification (ECMA-262) and the ECMAScript " \
                                                         "Internationalization API specification (ECMA-402). The JavaScript " \
                                                         "documentation throughout MDN is based on the latest draft versions of " \
                                                         "ECMA-262 and ECMA-402. And in cases where some proposals for new ECMAScript" \
                                                         " features have already been implemented in browsers, documentation and " \
                                                         "examples in MDN articles may use some of those new features.\n\nDo not " \
                                                         "confuse JavaScript with the Java programming language. Both \"Java\" and" \
...
def test_footer():
    driver = init()
    sleep(3)
    footer = driver.find_element(By.XPATH,"//body/div[@id='root']/div[1]/div[2]/div[3]/div[1]").get_attribute("innerText")
    sleep(3)
